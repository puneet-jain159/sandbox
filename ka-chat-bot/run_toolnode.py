...
def genie_query_tool(
    ...
    - The backend data contains rolls up at multiple levels of location hence it needs to be specified even if country is specified i.e country can be at cluster region level as well
...
- If the last tool output is a JSON object with {"type":"clarification_request","message":...},
  ask the user for exactly those missing details (e.g., "hierarchy") and do not invent values.
